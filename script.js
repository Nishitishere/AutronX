...
$(document).ready(function () {
  ...
  $("#auraPlusSec").click(function () {
    design = "color"; // Use color mode (same as Aura)
    $(".navbar").addClass("hide");
    $("#watermark").removeClass("hide");
    $(".right-bar").removeClass("hide");
    $(".rmenu-item:first-child").removeClass("disabled");
    allData.Panel = "Aura+";
    $("#auraPlusSec").addClass("button");
    $("#buildbtn").removeClass("button");
    $("#colorSec").removeClass("button");

    // Add both color and auraplus classes - color for behavior, auraplus for extended frame
    $('#maindiv').removeClass("edge");
    $('#maindiv').addClass("color");
    $('#maindiv').addClass("auraplus");
    ...
    if (boxSize > 0) {
      ...
      if ($(this).attr("title") == "curtain") {
        if (boxSize >= 2) {
          if (mod == 12) {
            if (boxSize > 6 && 6 >= box1Capacity + 2) {
              curtainBox1(2, ".sub-module-th", box1lis, "curtain", "Curtain");
            } else if (6 >= box2Capacity + 2) {
              curtainBox2(2, ".sub-module-fi", box2lis, "curtain", "Curtain");
            } else {
              customAlert("No Space availabe!");
            }
          } else {
            curtainModAll(2, "curtain", "Curtain");
          }
        } else {
          customAlert("Adding 2 Module item is not possible!");
        }
      ...
      if ($(this).attr("title") == "switch4-fan2") {
        if (boxSize >= 6) {
          if (mod == 12) {
            ...
            customerRequirement["sub-element-" + n] = 6;
            n++;
            allData["Accessories-" + obj] = "4 Switch + 2 Fan";
            obj++;
          }
        } else {
          customAlert("Adding 6 Module item is not possible!");
  ...
  $("body").on("click", ".closetwo", function () {
    ...
    const targetSelector = "div.sub-element-" + a[0] + ">div.sw-ele-" + a[1];
    ...
    $(".closetwo").addClass("show");
    const droppableElementData = $(targetSelector).attr("data-droppable-class");
    if (
      design == "color" ||
      droppableElementData === "fan" ||
      droppableElementData === "dimmer"
    ) {
      restoreDropTargetVisual(targetElement);
    } else {
      $(targetSelector).removeClass("hiden-border");
    }
  });
...
function getPanelTypeLabel(panelName) {
...
// Curtain functions with drop zones
function curtainModAll(bsize, addcls, dataName) {
  boxSize -= bsize;
  const node = document.createElement("div");
  node.classList.add("sub-element-" + n);
  $(".sub-module").append(node);
  node.classList.add(addcls);
  $(".sub-element-" + n).html(
    '<div class="curtain-drop sw-ele-1" ondrop="drop(event)" ondragover="allowDrop(event)" data-droppable-class="curtain"></div>'
  );
  closebtn(n);
  customerRequirement["sub-element-" + n] = bsize;
  n++;
...
function curtainBox1(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box1Capacity += bsize;
  const node = document.createElement("div");
  node.classList.add("sub-element-" + n);
...
function curtainBox2(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box2Capacity += bsize;
  const node = document.createElement("div");
  node.classList.add("sub-element-" + n);
...
function getMixedAccessorySlotTypes(layoutName) {
  if (layoutName === "switch4fan1") {
    return ["switch", "switch", "switch", "switch", "fan", "dimmer"];
  }
  if (layoutName === "switch4fan2") {
    return [
      "dimmer",
      "fan",
      "switch",
...
function getMixedAccessoryMarkup(layoutName) {
  const slotTypes = getMixedAccessorySlotTypes(layoutName);
  const switchClass = design === "color" ? "switch2Image" : "switch2";

  return slotTypes
    .map(function (slotType, index) {
      const slotNo = index + 1;
      const classes = [layoutName + "-" + slotNo, "sw-ele-" + slotNo];

      if (slotType === "switch") {
        classes.unshift(switchClass);
      } else if (slotType === "fan") {
        classes.unshift("switch2ImageFan");
      } else {
        classes.unshift("switch2ImageDimmer");
        classes.unshift("hiden-border");
        classes.unshift("fan");
      }
      ...
      return (
        '<div class="' +
        classes.join(" ") +
        '" ondrop="drop(event)"ondragover="allowDrop(event)" data-droppable-class="' +
        slotType +
        '"></div>'
      );
    })
...
function addMixedAccessoryCloseButtons(layoutName) {
  const slotTypes = getMixedAccessorySlotTypes(layoutName);
  slotTypes.forEach(function (slotType, index) {
    closetwo(n, index + 1, slotType === "dimmer" ? "closethree" : "closetwo");
  });
}

function appendMixedAccessoryNode(targetSelector, containerClass, layoutName) {
  const node = document.createElement("div");
  node.classList.add("sub-element-" + n);
  $(targetSelector).append(node);
  node.classList.add(containerClass);
  $(".sub-element-" + n).html(getMixedAccessoryMarkup(layoutName));
  addMixedAccessoryCloseButtons(layoutName);
  closebtn(n);
}

function Switch4fan1(n) {
  appendMixedAccessoryNode(".sub-module", "switch4fan1", "switch4fan1");
}
function switch4fan1box1(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box1Capacity += bsize;
  appendMixedAccessoryNode(mod12Name, addcls, "switch4fan1");
  boxlis["sub-element-" + n] = bsize;
  n++;
  allData["Accessories-" + obj] = dataName;
  obj++;
...
function Switch4fan2(n) {
  appendMixedAccessoryNode(".sub-module", "switch4fan2", "switch4fan2");
}
function switch4fan2box1(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box1Capacity += bsize;
  appendMixedAccessoryNode(mod12Name, addcls, "switch4fan2");
  boxlis["sub-element-" + n] = bsize;
  n++;
  allData["Accessories-" + obj] = dataName;
  obj++;
...
function Switch6fan1(n) {
  appendMixedAccessoryNode(".sub-module", "switch6fan1", "switch6fan1");
}
function switch6fan1box1(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box1Capacity += bsize;
  appendMixedAccessoryNode(mod12Name, addcls, "switch6fan1");
  boxlis["sub-element-" + n] = bsize;
  n++;
  allData["Accessories-" + obj] = dataName;
  obj++;
...
function Switch6fan2(n) {
  appendMixedAccessoryNode(".sub-module", "switch6fan2", "switch6fan2");
}
function switch6fan2box1(bsize, mod12Name, boxlis, addcls, dataName) {
  boxSize -= bsize;
  box1Capacity += bsize;
  appendMixedAccessoryNode(mod12Name, addcls, "switch6fan2");
  boxlis["sub-element-" + n] = bsize;
  n++;
  allData["Accessories-" + obj] = dataName;
  obj++;
...
function getDropTargetDefaultClass(targetElement) {
  ...
  if (droppableElementData === "fan") {
    return "switch2ImageFan";
  }
  if (droppableElementData === "dimmer") {
    return "switch2ImageDimmer";
  }
  return design == "color" ? "switch2Image" : "switch2";
}

function restoreDropTargetVisual(targetElement, imageUrl) {
  ...
  targetElement.classList.add(getDropTargetDefaultClass(targetElement));
  if (imageUrl) {
    $(targetElement).css("backgroundImage", "url(" + imageUrl + ")");
    targetElement.dataset.previewSrc = imageUrl;
  } else {
    $(targetElement).removeAttr("style");
    delete targetElement.dataset.previewSrc;
  }
}

function getIconSlot(targetElement) {
  const module = targetElement.closest('[class*="sub-element-"]');
  const moduleClass = module && [...module.classList].find((className) => /^sub-element-\d+$/.test(className));
  const elementClass = [...targetElement.classList].find((className) => /^sw-ele-\d+$/.test(className)) || "target";
  return `${moduleClass || "panel"}:${elementClass}`;
}

function rememberIconDrop(targetElement, sourceElement) {
  if (typeof allData === "undefined" || !sourceElement?.src) return;
  allData[`Icon-${getIconSlot(targetElement)}`] = JSON.stringify({
    source: sourceElement.src,
    type: sourceElement.getAttribute("data-draggable-class") || "switch",
  });
}
...
window.restoreAutronxPanelIcons = function (configuration) {
  const entries = Object.entries(configuration || {}).filter(([key]) => key.startsWith("Icon-"));
  for (const [key, rawValue] of entries) {
    ...
    } catch { /* Ignore malformed historical icon records. */ }
  }
};

document.addEventListener("touchend", function (event) {
  if (!window.autronxTouchDragId) return;
  const touch = event.changedTouches?.[0];
  const target = touch && document.elementFromPoint(touch.clientX, touch.clientY)?.closest("[data-droppable-class]");
  if (!target) return;
